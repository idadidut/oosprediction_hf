...
##-------------------------------excel preprocessing----------------------------
setwd("D:\\HappyFresh\\projects\\Dropbox\\14. OOS Mining\\farmers/")
...
a= read.table(temp[[1]], sep = ";", fill = NA, quote = "", header = T, as.is = )
...
fileName <-"D:\\HappyFresh\\projects\\Dropbox\\14. OOS Mining/oos_purchased_only_farmers.sql"
query = readChar(fileName, file.info(fileName)$size)
dat = dbGetQuery(con,query)

dat$supermarket <- as.factor(dat$supermarket)

alldat <- left_join(dat, finalexcel[,-(3:5)], by = c("supermarket" = "Store", "sku"="Product.Code", "tgl"="tgl"))

#a=alldat %>% group_by(sku) %>% mutate(totrow=n())

#alldat <- alldat %>% group_by(sku) %>% mutate(rn = row_number(), dday = wday(tgl)) %>% ungroup()

alldat$supermarket <- as.factor(alldat$supermarket)
...
traintest <- function(x, supermarket){
  ...
  a = rawfinal %>% group_by(group) %>% summarise(x= x[which.max(dif)[1]])
...
trainmod= traintest(train, "Farmers")
testmod = traintest(test, "Farmers")

finaltraintest = left_join(trainmod, testmod, by = c("supermarket"="supermarket", "x"="x"))
MAE= finaltraintest %>% group_by(supermarket) %>% summarise(mae = mean(abs(y.x.x - y.x.y), na.rm = T))

write.table(finaltraintest, "clipboard-16384", sep="\t", row.names = F)
write.table(MSE, "clipboard-16384", sep="\t", row.names = F)
